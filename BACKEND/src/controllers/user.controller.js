...
// Get recommended users (not friends and not the current user)
...
export async function acceptFriendRequest(req, res) {
  try {
    ...
    //varify the current user is the recipient of the request
    if (friendRequest.recipient.toString() !== req.user.id) {
      return res.status(403).json({
        message: "You are not authorized to accept this request this request",
      });
    }
